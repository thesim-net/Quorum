 ...
 *
 * One registered redirect URI serves three purposes: signing in, attaching a
 * Discord identity to the account already in the session, and proving an
 * identity for a survey gated on the guild. Which one is running is carried in
 * the OAuth `state` parameter, and what the callback does with the result is
 * decided here.
 *
 ...
 *
 * A respondent sign-in is the same rule taken seriously rather than an
 * exception to it: it resolves to an identity and nothing else, so a gated
 * survey can check the guild without anybody's Discord id becoming an account.
 *
...
export function resolveCallback({
  ...
  // Taking a survey is not signing in. Whoever already holds the id, whether
  // they are an admin, and whether they are signed in here are all irrelevant:
  // the identity is handed back and no account is read, changed, or created.
  if (intent === INTENTS.RESPONDENT) {
    return { action: CALLBACK_ACTIONS.RESPONDENT, userId: null };
  }
